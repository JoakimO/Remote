...
public class SelectRemoteDeviceRemoteCommand implements RemoteCommand, RemoteDeviceListener {
	...
	public int execute() {
		...
		RemoteModelParameters params = remoteModel.getRemoteModelParameters(device, IDENTIFIER);
		if (params.containsParam("value")) {
			RemoteDevice remoteDevice = (RemoteDevice)params.getObjectParam("value");
	...
	public void remoteDeviceInitialized(RemoteDevice remoteDevice) {
		...
		RemoteDeviceBaseAdapter adapter = new RemoteDeviceBaseAdapter(R.layout.standard_device_spinner_item);
		...
		if (remoteModel.getSelectedRemoteDevice() != null) {
			params.putObjectParam("selectionPosition", remoteDevices.indexOf(remoteModel.getSelectedRemoteDevice()));
		}
