...
public class BoxeeRemoteDeviceDiscoverer implements RemoteDeviceDiscoverer {
	...
	public BoxeeRemoteDeviceDiscoverer(RemoteDeviceFactory remoteDeviceFactory) {
		...
		preferenceListener = new SharedPreferences.OnSharedPreferenceChangeListener() {
			public void onSharedPreferenceChanged(SharedPreferences prefs,
					String key) {
				if (key.equals("is_broadcast")
						|| key.equals("is_boxee_broadcast")) {
					if (prefs.getBoolean("is_broadcast", true)
	...
	public RemoteDevice createRemoteDevice(RemoteDeviceDetails details) {
		...
		if (details.getIdentifier().startsWith(APPLICATION)) {
			...
			for (BoxeeRemoteCommand.Command command : BoxeeRemoteCommand.Command
					.values()) {
				commands.add(new BoxeeRemoteCommand(device, command));
			}
			RemoteApplication remoteApplication = RemoteApplication
					.getInstance();
			remoteApplication.getRemoteCommandFactory().registerCommands(
					device.getIdentifier(), commands);

			RemoteDisplayFactory displayFactory = remoteApplication
					.getRemoteDisplayFactory();
			BoxeeRemoteDisplay remoteDisplay = new BoxeeRemoteDisplay(device);
			displayFactory.registerDisplay(device.getIdentifier(),
					remoteDisplay);
			BoxeeBrowserRemoteDisplay browserRemoteDisplay = new BoxeeBrowserRemoteDisplay(
					device);
			displayFactory.registerDisplay(device.getIdentifier(),
					browserRemoteDisplay);
	...
	private InetAddress getBroadcastInetAddress() {
		try {
			if (RemoteApplication.getInstance().isEmulator()) {
				byte[] quads = new byte[] { (byte) 192, (byte) 168, (byte) 0,
						(byte) 101 };
				// byte[] quads = new byte[]{(byte)192, (byte)168, (byte)0,
				// (byte)159};
				// byte[] quads = new byte[]{(byte)192, (byte)168, (byte)0,
				...
				WifiManager wifi = (WifiManager) RemoteApplication
	...
	private void sendDiscoveryRequest(DatagramSocket socket) throws IOException {
		String data = String
				.format("<?xml version=\"1.0\">\n<BDP1 cmd=\"discover\" application=\"iphone_remote\" challenge=\"%s\" signature=\"%s\"/>",
						this.challenge, getSignature(this.challenge));
		Log.v(TAG, "Sending data " + data);

		DatagramPacket packet = new DatagramPacket(data.getBytes(),
				data.length(), getBroadcastInetAddress(), DISCOVERY_PORT);
		socket.send(packet);
	}
	...
	/**
	 * This method will check the shared preferences if the thread should be
	 * broadcasting.
	 * 
	 * @return
	 */
	protected boolean isBroadcast() {
		SharedPreferences preferences = PreferenceManager
				.getDefaultSharedPreferences(RemoteApplication.getInstance()
						.getContext());
	...
	public boolean isEnabled() {
		...
		return preferences.getBoolean("is_boxee_enabled", false);
	}
}
